...
pm_1_ebc_df = pd.read_csv(r'C:\LocalData\sujaiban\sujai.banerji\Aerosol Optical Properties\beijing_hyytiala_winter_school\2018_2021\hyytiala\ae33_abs_bc_pm1_SMEARii_2018_2021.txt', sep = '\s+', header = None)
...
wavelengths = [370, 440, 520, 550, 660, 880, 950]
...
slope = pd.DataFrame({'slope': [np.exp(model.fit(x.values.reshape(-1, 1), y.iloc[:, idx]).coef_[0]) for idx in range(y.shape[1])]})
